...
class Strategy:
    ...
    async def get_current_price(self) -> int:
        current_price = await self.broker.current_price(creds.instrument, "CBOE")
    ...
    async def place_call_order(self, side: str):
        ...
        while self.atm_call_fill > 0:
            ...
            if not self.should_continue:
                ...
                if creds.close_hedges and side == "SELL" and creds.active_close_hedges and self.otm_call_id is None:
                    await self.place_order(side="SELL", type="C", strike=hedge_target_price,
    ...
    async def main(self):
        ...
        if self.func_test:
            await self.broker.cancel_order(5)
            return
